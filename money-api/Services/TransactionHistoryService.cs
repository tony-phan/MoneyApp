...
public interface ITransactionHistoryService
{
    ...
    Task<bool> DeleteTransactionHistory(int id);
}
...
public class TransactionHistoryService : ITransactionHistoryService
{
    ...
    public async Task<TransactionHistoryDto> Create(TransactionHistoryCreateDto transactionHistoryCreateDto)
    {
        ...
        if (exists) throw new DuplicateTransactionHistoryException();

        var user = await _accountRepository.GetById(transactionHistoryCreateDto.UserId);
        if (user == null)
        {
            throw new Exception($"User with ID {transactionHistoryCreateDto.UserId} not found.");
        }
    ...
    public async Task<bool> DeleteTransactionHistory(int id)
    {
        var th = await _transactionHistoryRepository.GetById(id);
        if (th == null) return false;

        if (th.Transactions != null)
        {
            _transactionRepository.DeleteRange(th.Transactions);
        }

        await _transactionHistoryRepository.Delete(id);
    ...
    {
        var tH = await _transactionHistoryRepository.GetAll();
        return tH.Select(_mapper.Map<TransactionHistoryDto>).ToList();
    }
